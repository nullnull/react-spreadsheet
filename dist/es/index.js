import { createElement, useRef, useCallback, useEffect, PureComponent } from 'react';
import createStore from 'unistore';
import devtools from 'unistore/devtools';
import { connect, Provider } from 'unistore/react';
import { columnIndexToLabel, extractLabel, Parser } from 'hot-formula-parser';
...
var Table = function (_a) {
    ...
    var columnNodes = range(columnCount).map(function (i) { return createElement("col", { key: i }); });
    return (createElement("table", { className: "Spreadsheet__table" },
        createElement("colgroup", null, columnNodes),
...
function CornerIndicator(props) {
    return createElement("th", { className: "Spreadsheet__header" });
}

var ColumnIndicator = function (_a) {
    var column = _a.column, label = _a.label;
    return (createElement("th", { className: "Spreadsheet__header" }, label !== undefined ? label : columnIndexToLabel(String(column))));
};

var RowIndicator = function (_a) {
    var row = _a.row, label = _a.label;
    return (createElement("th", { className: "Spreadsheet__header" }, label !== undefined ? label : row + 1));
};
...
var Cell = function (_a) {
    ...
    return (createElement("td", { ref: rootRef, className: classnames("Spreadsheet__cell", data === null || data === void 0 ? void 0 : data.className, {
        ...
        createElement(DataViewer, { row: row, column: column, cell: data, formulaParser: formulaParser })));
};
...
var enhance = connect(mapStateToProps, function () { return ({
    select: select,
    activate: activate,
...
var toView = function (value) {
    if (value === false) {
        return createElement("span", { className: "Spreadsheet__data-viewer--boolean" }, "FALSE");
    }
    if (value === true) {
        return createElement("span", { className: "Spreadsheet__data-viewer--boolean" }, "TRUE");
    }
    return createElement("span", { className: "Spreadsheet__data-viewer" }, value);
};
...
var DataEditor = function (_a) {
    ...
    return (createElement("div", { className: "Spreadsheet__data-editor" },
        createElement("input", { ref: inputRef, type: "text", onChange: handleChange, value: value, autoFocus: true })));
};
...
function ActiveCell(props) {
    ...
    var handleChange = useCallback(function (cell) {
    ...
    return hidden ? null : (createElement("div", { className: classnames("Spreadsheet__active-cell", "Spreadsheet__active-cell--" + mode), style: { width: width, height: height, top: top, left: left }, onClick: mode === "view" && !readOnly ? edit : undefined }, mode === "edit" && (createElement(DataEditor, { row: row, column: column, cell: cell, 
        // @ts-ignore
...
var FloatingRect = function (_a) {
    ...
    return (createElement("div", { className: classnames("Spreadsheet__floating-rect", (_b = {},
...
var Selected = function (props) { return (createElement(FloatingRect, __assign({}, props, { variant: "selected" }))); };
...
var Copied = function (props) { return (createElement(FloatingRect, __assign({}, props, { variant: "copied" }))); };
...
var Spreadsheet = /** @class */ (function (_super) {
    ...
    Spreadsheet.prototype.render = function () {
        ...
        return (createElement("div", { ref: this.handleRoot, className: "Spreadsheet", onKeyPress: onKeyPress, onKeyDown: this.handleKeyDown, onMouseMove: this.handleMouseMove },
            createElement(Table$1, { columns: columns, hideColumnIndicators: hideColumnIndicators },
                createElement(Row$1, null,
                    !hideRowIndicators && !hideColumnIndicators && createElement(CornerIndicator$1, null),
                    !hideColumnIndicators &&
                        range(columns).map(function (columnNumber) {
                            return columnLabels ? (createElement(ColumnIndicator$1, { key: columnNumber, column: columnNumber, label: columnNumber in columnLabels
                                    ? columnLabels[columnNumber]
                                    : null })) : (createElement(ColumnIndicator$1, { key: columnNumber, column: columnNumber }));
                        })),
                range(rows).map(function (rowNumber) { return (createElement(Row$1, { key: rowNumber },
                    !hideRowIndicators &&
                        (rowLabels ? (createElement(RowIndicator$1, { key: rowNumber, row: rowNumber, label: rowNumber in rowLabels ? rowLabels[rowNumber] : null })) : (createElement(RowIndicator$1, { key: rowNumber, row: rowNumber }))),
                    range(columns).map(function (columnNumber) { return (createElement(Cell$1, { key: columnNumber, row: rowNumber, column: columnNumber, 
                        // @ts-ignore
                        DataViewer: DataViewer$1, formulaParser: _this.formulaParser })); }))); })),
            createElement(ActiveCell$1
            // @ts-ignore
            , { 
                // @ts-ignore
                DataEditor: DataEditor$1, getBindingsForCell: getBindingsForCell$1 }),
            createElement(Selected$1, null),
            createElement(Copied$1, null)));
    };
    return Spreadsheet;
}(PureComponent));
...
var Spreadsheet$1 = connect(mapStateToProps$4, {
    copy: copy,
    cut: cut,
    paste: paste,
...
var SpreadsheetStateProvider = /** @class */ (function (_super) {
    ...
    SpreadsheetStateProvider.prototype.render = function () {
        ...
        return (createElement(Provider, { store: this.store },
            createElement(Spreadsheet$1, __assign({}, rest, { store: this.store }))));
    };
...
export { DataEditor, DataViewer, SpreadsheetStateProvider as Spreadsheet, createEmptyMatrix, getComputedValue };
